    ...
    <div class=" bg-white dark:bg-dark-card  dark:bg-dark-card rounded-[0.5rem] shadow-card">
        ...
        <div class="px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-border dark:border-dark-border">
            ...
            <div class="flex items-center gap-1">
                ...
                @else
                    <a href="{{ $petani->previousPageUrl() }}" class="w-8 h-8 flex items-center justify-center rounded-[0.375rem] border border-border dark:border-dark-border text-muted dark:text-dark-muted hover:bg-slate-100 dark:hover:bg-dark-border/30 transition-colors">
                        <span class="material-symbols-outlined text-[18px]">chevron_left</span>
                    </a>
                @endif
                @foreach($petani->getUrlRange(1, $petani->lastPage()) as $page => $url)
                    <a href="{{ $url }}" class="w-8 h-8 flex items-center justify-center rounded-[0.375rem] {{ $page == $petani->currentPage() ? 'bg-primary text-white font-medium shadow-primary' : 'text-body dark:text-dark-body hover:bg-slate-100 dark:hover:bg-dark-border/30' }} transition-colors font-medium text-[13px]">{{ $page }}</a>
                @endforeach
                @if($petani->hasMorePages())
                    <a href="{{ $petani->nextPageUrl() }}" class="w-8 h-8 flex items-center justify-center rounded-[0.375rem] border border-border dark:border-dark-border text-muted dark:text-dark-muted hover:bg-slate-100 dark:hover:bg-dark-border/30 transition-colors">
